    ...
    <>
      <div className="flex justify-between items-end">
        ...
        <div className="italic text-xs">
          {dateParser(work.startDate)} - {work.isWorkingHere ? 'sekarang' : dateParser(work.endDate)}
        </div>
      </div>
      <div className="flex justify-between items-end">
        <div className="font-medium text-xs">{work.position}</div>
        <div className="italic text-xs">{work.years.replace('years', 'tahun').replace('year', 'tahun')}</div>
      </div>
    </>
  );
}
